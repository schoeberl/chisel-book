...
Therefore, we recommend to skip Chisel 5 and use Chisel 6.
Chisel 6 might switch to a different simulation and testing library in the future.
We use Scala 2.13 and tested with Java 17 and Java 21.
...
The number of pipeline stages is an architecture definition. Longer pipelines may allow
to clock the design at a higher frequency. However, with each additional pipeline stage
the overhead due to the registers increase and the design becomes more complex.
...
One aspect that determines the number a pipeline stages is the available on-chip memory
for instruction and data scratchpad memory or instruction and data caches.
Current on-chip memories have registers at their inputs (address and write data),
see Section~\ref{ref:memory}. That input register is part of the pipeline, it is the
...
assume scratchpad memories for instruction memory (IM), the register file (RF),
and the data memory (DM). Those three on-chip memories setting a lower bound
on the stages to three.\footnote{If we build the RF out of concrete flip-flops, we can
reduce the number of stages to two, as we can read asynchronous from such
a RF.}



Instructions flow from the
left to the right. And a result of an instruction is written back from right to left
either into the register file (RF) or the data memory (DM).
