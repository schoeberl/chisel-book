...
\todo{Look into the notes, maybe also into git log/diff}
...
Extend the processor chapter with more details.
...
on-chip register. Access to memory (load or store) is also performed via
...
memory access is stored in an address register.
...
or jump instruction.
Leros has the unconditional and conditional branch instructions.
...
\todo{Example assembler instructions}

\todo{Maybe list the encoding.}
...
\todo{draw the datapath}

\todo{the datapath showas a single memory for data and register, the Chisel code has two memories.}

\begin{figure}
  \centering
  \includegraphics[scale=\scale]{figures/leros-datapath}
  \caption{Leros.}
