...
This book has been written without any help of artificial intelligence and large language models
to either write or rewrite text. We used Grammarly\footnote{\url{https://app.grammarly.com/}}
...
In an FPGA on-chip memories are initialized to zero at configuration time.
...
to return a 0 and not the random content of the on-chip memory.
Note that the \code{read()} function returns the result with one clock
cycle latency. Therefore, the multiplexer at the output of the memory
needs to compare against the register address delayed by once clock cycle.
...
values.\footnote{It also returns debugging registers as well,
...
Forwarding is needed if we use a register for the address that was just written by
the preceding instruction. In that case, the value is not yet written back into the register
