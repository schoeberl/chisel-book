...
detection with a Moor FSM. 

\begin{figure}
  \centering
  \includegraphics[scale=\scale]{figures/state-diag-rising-moore}
  \caption{The state diagram of the rising edge detector as Moor FSM.}
  \label{fig:diag:rising:moore}
\end{figure}

Figure~\ref{fig:diag:rising:moore} shows the state diagram for the rising
edge detection with a Moor FSM. Fist thing to notice is that the Moor FSM
...
Listing~\ref{lst:fsm:rising:moore} shows the Moor version of the rising edge detection
...
FSMs as they need less state (and therefore logic) and react faster than a Moor FSM.
...
A classic example is a traffic light controller (see~\cite[Section~14.3]{dally:vhdl:2016}).
...
To make this example a little bit more interesting, have a priority road.
The other road has two car detectors (on both entries into the intersection).
Switch to green for this road only when a care is detected and then switch
