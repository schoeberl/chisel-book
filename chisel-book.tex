...
the end of the list. This list of states is a \code{Enum} type \emph{assigned}.
...
On a Moor FSM the output depends only on the current state.
...
The output becomes 1 for one clock cycle then the current input is 1
...
Similar to the Moor FSM, the register contains the current \code{state} and
the next state logic computes the the next state value (\code{next\_state})
from the current \code{state} and the input (\code{in}).
On the next clock tick, \code{state} becomes \code{next\_state}.
...
only two states are possible, which we name \code{A} and \code{B} in this
example.
...
Note also the we draw self transitions, e.g., that in state \code{A} when
the input it \code{0} the FSM stays in state \code{A} and the output is {0}.
The rising edge FSM generates the \code{1} output only on the transition
from state \code{A} to state \code{B}. In state \code{B}, which represents
that the input is now \code{1}, the output is \code{0}. We only want a single
puls for each rising edge.

\todo{Do the edge detection as Mealy machine, show Chisel, HW and than States,
and than a Chisel FSM code, plus discussion of what size FSM shall be.}

\section{Moore versus Mealy FSM}

\todo{Redo the edge detector. Maybe have a timing diagram.}
