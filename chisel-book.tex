...
\section{A Classic Processor Bus}
...
To select between different devices upper bits of the address bus are the
...
As the data bus is bi-directional and the data lines are shared between al devices,
...
Modern computers have different busses for different peripherals, e.g., a dedicated memory
...
The concept of a bus is also used on-chip, but with some adaptions.
Shared busses and the needed tri-state drivers are not practical within a chip. Furthermore,
wires in a chip are cheaper than wires on a PCB or a connector. Therefore, the data bus
is split into input and output signals.
The output of the peripherals is selected with a multiplexer. 
The address decoder drives the select of the multiplexer.
...
connection. Masters and devices are connected with such a point-to-point
interface to a switching fabric.
