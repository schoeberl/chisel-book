...
For the third edition I would like to acknowledge Hans Jakob Damsgaard for rewriting
...
\noindent Note that the slides are not automatically compiled with that \code{Makefile}, but need
the generated code and figures from the book. 
...
\noindent ScalaTest enables simple unit tests that read like an executable specification.
...
\code{sbt test} executes all available test, which is useful for regression
test.\footnote{Try \codefoot{sbt test} in the repository of this book and you
will see more than 70 tests passing.}
However, if you want to run a single test (suit) you can do this with:
...
there will be a relative silent error message: \code{No tests were executed}.
...
\subsection{WaveForm and Printing from below}
