...
It is possible in Eclipse and IntelliJ to generate a project from the sbt project configuration
in \code{build.sbt}.
...
These basic elements can be combined to build larger, more interesting circuits.
...
\noindent When you find the notion of 8.U and 4.W a little bit funny, consider it as variant of an integer
constant with a type. This is similar to 8L, representing a long integer constant in C, Java, and Scala.

Chisel benefits from Scala's type inference and in many places type information can be left out.
The same is true for bit width. In many cases Chisel will automatically infer the correct width.
...
\noindent In this example we do not define the type nor the width of signal \code{logic}.
...
With logical and arithmetical operations and a multiplexer basically every combinational
circuit can be described. However, Chisel provides further components and control abstractions
for a more elegant description of a combinational circuit, which are described late.
...
Copy that project into a new folder and extend it by adding some inputs to the \code{Bundle}
...
Is yes, you have now inputs available to play with. If not, you need to debug your FPGA configuration.
...
in real hardware in an FPGA. As a next step we will et a first look how the build
process works to generate a FPGA configuration. Furthermore, we will also
explore a simple testing framework from Chisel, which allows you to test circuits
with configuring an FPGA and toggle switches.
...
Packages organize you Chisel code into namespaces. Packages can also contain
sub-packages.
...
\todo{Describe minimum sbt commands (run main, Chisel main code, use target folder generated
as an example for options.}
...
simply called DTU), (2) the testing logic, also called test bench, and (3) the tester objects
...
step (= one clock cycle) with \code{step()}.

\todo{First do just print, then also have a complete testing example}
