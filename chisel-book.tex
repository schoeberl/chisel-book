...
blocks that can be synthesized in an \myref{todo}{ASIC}.
...
However, it is intended to teach digital design at an abstraction level that is
current practice to describe ASICs or designs targeting
\myref{todo}{FPGA}s.\footnote{As the author is more familiar with FPGAs
than ASICs as target technology, some design optimizations shown in this book are targeting FPGA
technology.}
...
All code examples shown in the book are extracted from complete programs
...
This book is optimized to be read electronically on a laptop or on a tablet (e.g., iPad).
