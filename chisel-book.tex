...
signal (\code{rd}) in clock cycle 3. Signal \code{ack} needs to react
...
one clock cycle after the \code{rd} command, in clock cycle 3.
The first read sequence has the same latency as the former example.
...
Table~\ref{tab:addr:uart} shows an address mapping for our UART. At the base address (0xf000)
we access a status register on a read and an optional control register on a write.
At the next address (0xf001) we read from a possible read buffer and write into
a transmit buffer. 
...
Both bits signal a that we can perform a write or a read. When the transmit data register
