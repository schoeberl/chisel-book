...
Chisel is perfectly fine to be used as first hardware description language.
...
\code{D} and an output \code{Q}. Each register also contains an input for a clock signal.
...
A FSM has an initial state, which is set on reset.
...
The output logic computes the output (\code{out}). As the output depends only on the current
state, this state machine is called a
...
the synthesize tool and aim for readable code. Therefore, we use an enumeration
type with symbolic names for the states:

\input{code/simple_fsm_states.txt}

\noindent The register holding the state is defined with the \emph{green} state as the reset value:

\input{code/simple_fsm_register.txt}
...
logic, which depends on out inputs, by assigning a new value for our state register:

\input{code/simple_fsm_next.txt}

\noindent Lats, but not least we code our \emph{ringing bell} output to be true when
the state is \emph{red}.

\input{code/simple_fsm_output.txt}
...
Figure~\ref{fig:mealy} shows a Mealy type FSM. The register contains the current \code{state}.

\todo{Do the edge detection as Mealy machine, show Chisel, HW and than States}

\section{Moore versus Mealy FSM}

\section{Exercise}

\todo{Maybe edge detection as Moor machine plus evaluation of the effects.}
...
The light flasher has one input \code{start} and an output
...
\input{code/flasher_timer.txt}

\noindent Following code shows the master FSM:

\input{code/flasher_fsm.txt}
...
\input{code/flasher2_counter.txt}
