...
An input signal may come from a source that has not a clean transition from
...
The later two issues, debouncing switches and filtering noise, can also be
...
\todo{read Dally on this topic to check}
...
Metastability cannot be avoided, but we can contain it.
...
from a synchronous output signal and the maximum propagation delay
is know. A classic example is interfacing an asynchronous SRAM
to a synchronous circuit, e.g., a microprocessor.}
This also includes an external reset signal. The reset signal
shall pass through the two flip-flops before it is used as reset signal
for other flip-flops in the circuit.

\section{Debouncing}

Switches and buttons may need some time transition between on and off.
...
transition events then we intend. One solution is to use time to filter out
this bouncing. Assuming a bouncing time of $t_{bounce}$ we will sample the
...
to the still 0 samples or to the already 1 samples. The important point
is that we have only one transition from 0 to 1.

\begin{figure}
  \centering
  \includegraphics[scale=\scale]{figures/debounce}
  \caption{Debouncing in input signal.}
  \label{fig:debounce}
\end{figure}

Figure~\ref{fig:debounce} shows the sampling for the debouncing in action.
The top signal shows the bouncing input and the arrows below the sampling
points. Note that the distance between those sampling points need to longer
than the bouncing time. The first sample safely samples a 0 and the last
sample in the figure safely samples a 1. The middle sample falls into the
...
The Chisel code for the debouncing is a little bit more evolved than the synchronizer.
...
First we need to decide on the sample frequency. The above example
assumes a 100~MHz clock and result in a sample frequency of 100~Hz
...
register stores the input sample. In our example the signal is named
...
\section{Filtering and Signal Processing}

\section{Combining the Input Processing with Functions}

\section{Exercise}

Build a complete counter that is incremented by an input button.
Display the counter value at LEDs on an FPGA board.
Build the complete input processing chain with: (1) input synchronizer,
(2) sampling for debouncing, (3) majority voting to suppress noise,
and (4) an edge detection to trigger the increment of the counter.

As modern buttons and switches might not ``reliable'' bounce you can
simulate the bouncing and the spikes by pressing the button manually and
using a low sample frequency. Select 1 second as sample frequency,
i.e., if the input clock runs at 100~MHz, divide by 100,000,000.
...
debouncing circuit sampling with 1~Hz.
...
majority voted in is only recognized when it is longer than 1--2 seconds.
