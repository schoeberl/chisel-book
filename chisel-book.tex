...
which contains the hardware sources, and \code{test}, which contains tests.
Chisel inherits from Scala, which itself inherits from Java, the organization of source files
...
To execute tests based on ChiselTest and ScalaTest you can simply run them with
\begin{verbatim}
$ sbt test
\end{verbatim}
...
Chisel provides two testing methods: \myref{https://www.chisel-lang.org/chiseltest/}{ChiselTest}
in package \code{chiseltest} and the older \code{PeekPokeTester} in package
\code{iotesters}.\footnote{\codefoot{iotesters} has been deprecated since Chisel 3.5.
% FIXME: The referenced appendix does not exist.
A description of how to use the \codefoot{PeekPokeTester} can be found in Appendix~\ref{chap:peekpoke}}
...
use the command line options.} % It is not clear what this means. How is the ability to use a waveform viewer an alternative to command line options?
...
into a \code{Module()} call, and assigning them the names \code{add} and \code{reg}.
...
\code{0} and \code{3} and the condition \code{cond} serves as the select signal.
...
that single cycle rising edge signal to increment a counter.
...
When data arrives, signaled with an asserted \code{dinValid}, the FSM loads the shift register
...
and proper indexing. This code snippet is just to show how a Scala for loop can be used
...
\noindent Again, this is a too short an example to be useful, as \code{RegNext()}
...
The value \code{res} points to the bundle containing the minimum value and the position.

\shortlist{code/fun_min2.txt}

As a final variation of the minimum search circuit, we will use more Scala features
...
signal (\code{rd}) in clock cycle 3. The signal \code{ack} needs to react
...
Maybe then compare. This needs some better writing, also put it into the soc-comm README.}
 ...
 A read or write command is signaled by an asserted \code{rd} or \code{wr}.
...
is only a single cycle long. The address bus and the read signal do not need to be driven
...
\todo{Maybe mention the soc-comm project?}
...
In our example system all devices, being it memory or IO devices, are connected to shared
...
Listing~\ref{lst:rv-device} shows the \todo{continue here}
...
Holding the data in the slave is usually \emph{free} with respect to
...
clock cycle, and then the write signal deasserted. Also here we wait in an endless loop for
\code{ack} to become true.

With those three functions available, we can write more readable tests in fewer lines of code.
...
The benefit of Treadle is the quick startup time and the fact that no additional tools need to be installed.
