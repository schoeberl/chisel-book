...
We tested the book with following Chisel Versions: 3.5.6, 3.6.0, and 5.1.0.
\todo{5.1.0 fails on memory loading....}
Chisel version 6.0.0-RC1 is not supported by the latest ChiselTest version.
...
\todo{Is ChiselTest still supporting non-Verilator flow, right?}
Chisel 6 might not be supported by ChiselTest \todo{Kevin said he will.}

We use Scala 2.13 and tested with Java 17. Note that Java 21 is currently not supported
by \code{sbt}.

Chisel is recently going through some major changes. Up to Chisel 3.x, Chisel is mainly
...
\todo{This might go into getting started.}
...
All types basically represents a collection of bits. Therefore, we can easily preform this mapping.
...
\longlist{code/sv_ch_adder.txt}{A simple component in Chisel.}{lst:sv:ch:adder}
...
\longlist{code/sv_adder.txt}{A simple component in SystemVerilog.}{lst:sv:adder}
...
The main body of the module uses the SV construct \code{always\_comb}
...
\longlist{code/sv_use_chisel_adder.txt}{Using the \code{Adder} component in Chisel.}{lst:ch:use}

\longlist{code/vhdl_use_adder.txt}{Using the \code{adder} component in VHDL.}{lst:vhdl:use}

\longlist{code/sv_use_adder.txt}{Using the \code{Adder} component in SV.}{lst:sv:use}
...
Listing~\ref{lst:sv:use} shows how to create and use a component (module)
in SystemVerilog. Here it is created by calling it, followed by a name (\code{m}),
...
In VHDL and SV, the name of the component is not used so often, in Chisel we need
...
Listing~\ref{lst:sv:ch:register} shows an 8-bit register with reset (to 0) and an enable input in Chisel,
...
Listing~\ref{lst:sv:register} shows the definition of a
...
Registers are defined implicitly by a code pattern in SV
and VHDL. Whereas, in Chisel the registers are defined explicitly.

\subsection{Advanced Chisel Features}

The basic elements look similar in Chisel, SV, and VHDL, and the verbosity
is also in the same range (although VHDL is a bit chattery).
However, neither VHDL nor SV contain object-oriented features for hardware
description. SV include object-oriented programming only for writing test benches.
...
Although any hardware can be equally described in Chisel, VHDL, and SV,
