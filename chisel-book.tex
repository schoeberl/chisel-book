...
\ \\
\vspace{1cm}
{\usekomafont{title}\mdseries\Large Seventh Edition}
\cleardoublepage
\end{flushleft}
\newpage
...
\bigskip
{\usekomafont{title}\huge Seventh Edition}\\
\bigskip
\bigskip
\bigskip
\bigskip
...
This book is an introduction to digital design with a focus on using the hardware construction language Chisel. Chisel brings advances from software engineering, such as object-oriented and functional languages, into digital design.
...
I have not used any large language model (LLM) to write even a single sentence. All mistakes are mine, not a hallucination by an LLM. I use Grammarly for grammar checking and Copilot for writing code.

% about me, backside
% TODO: update the text at Amazon

%Martin Schoeberl is Professor at the Technical University of Denmark, where he is teaching digital electronics, computer architecture, and chip design. His research interest is on hard real-time systems, time-predictable computer architecture, and real-time Java. He has more than 100 publications in peer-reviewed journals, conferences, and books.

%Martin has been several times at UC Berkeley on research stays, where he has picked up Chisel and was in close contact with the developers of Chisel. He lead the research project T-CREST where most of the components have been written in Chisel.

\section*{Foreword for the Second Edition}

As Chisel allows agile hardware design, so does open access and on-demand printing,
which allows agile textbook publishing. Less than 6 months after the first edition of this book
...
As there are still many Chisel designs available that use the \code{PeekPokeTester}, we have moved the description for it into the appendix.
...
We have added an arbiter, priority encoder, and comparator to the chapter on
...
To make room for more advanced topics, I have removed the two appendices on
...
If needed, those two chapters are available in the older versions of the Chisel book, available as PDF at the \myref{https://www.imm.dtu.dk/~masca/chisel-book.html}{web page} for this book.
...
After 3.x, the architecture of the Chisel compile pipeline has changed. To indicate that change,
the version numbering system has changed to increment the major version number more often.
...
Chisel 3.6 is the last version that uses the Scala-based backend to generate Verilog files.
...
Therefore, we recommend that Chisel 5 be skipped and Chisel 6 be used.
...
We extended the testing chapter with a description of how to access internal signals and
how to use assertions. Furthermore, we started to explore formal verification in Chisel.

Organizing digital circuits as pipelines to increase performance is a key aspect of digital design
...
This book was written without any help from artificial intelligence or large language models
...
\section*{Foreword for the Seventh Edition}
...
If you are interested in translating this book into another language, feel free to do it and
...
Chisel brings advances in software engineering, such as object-oriented
and functional programming in the digital design domain.
Chisel not only allows you to express hardware at the register-transfer level
but also allows you to write hardware \emph{generators}.
...
This book is freely available as a PDF eBook and in a classical printed version from
...
Here is a list of further reading for digital design and Chisel:
\begin{itemize}
...
The next step is to set up an FPGA project file for the synthesis tool, assign the pins,
compile\footnote{The real process is more elaborate with following steps: synthesizing the logic,
...
also use the following terms: low/high, false/true, and deasserted/asserted.
...
\noindent With the parentheses, we wrap the two wires that are connected to the outputs of
...
Scala has a very lightweight construct to package several fields into a class:
...
An option can return a value wrapped in \code{Some} or represent the missing value
...
\todo{Sample code for a peripheral, how to test? Explain figure (text from other book)}

\begin{figure}
  \centering
  \includegraphics[scale=1]{figures/wishbone}
  \caption{Wishbone asynchronous read followed by an asynchronous write.}
  \label{fig:wishbone}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[scale=1]{figures/wishbone_sync}
  \caption{Wishbone synchronous read followed by a synchronous write.}
  \label{fig:wishbone:sync}
\end{figure}
...
components, we call it formal verification.
In this book, we will stick to the term \emph{testing} when we write tests for our hardware description.
Verification (testing) can be performed as dynamic verification by executing the design on a simulator or as formal verification with a model checker or an SMT solver.
