...
Interconnect, arbitration circuit (combinational and functional programming), 
...
Figure~\ref{fig:vec-mux} shows the resulting schematic of the above code snippet.
...
This includes, e.g., writing a reference model of your hardware in Scala to test against.
...
%and where the iotesters methods operate on Scala type inputs (i.e., \code{BigInt}s and 
...
expression.\footnote{Scala also contains a \code{return} statement. The code could have been written
a bit more verbose as \code{return x + y}.}
...
\begin{figure}
  ...
  \caption{A read transaction with an acknowledge.}
  \label{fig:bus_ack}
\end{figure}
...
\begin{figure}
  ...
  \caption{Read transaction with a later acknowledgement.}
...
To implement the delayed acknowledge, we use a single bi register (\code{ackReg}) and
simple delaying any asserted \code{rd} or \code{wr}. As we provide the read result in
...
The counters themself are a small register file of 4 elements (a \code{Reg} of a \code{Vec}).
...
The counter are freely running and increment by one each clock cycle, except when written
a new value.
