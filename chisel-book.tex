...
libraries, and generates the Java class \code{Hello.class} that ca be executed by a standard
...
kitchen timer: you set the timeout value and press start. When the timeout has
expired the alarm goes off.
...
the value of \code{din} and asserting \code{load}. When the \code{load} signal is de-asserted
counting down is selected (by selecting \code{cnt - 1} as the input for the register).
...
is a signal with a constant period and a modulation of the time the signal is \emph{on}
within that period.
...
Figure~\ref{fig:pwm} shows a signal with PWM. The arrows point to the start of the periods
of the signal. The percentage of time the signal is one, is also called the duty cycle.
...
The function has two parameters: a Scala integer configuring the PWM with the number of clock cycles,
and a Chisel wire that gives the threshold for the PWM output signal. We use a multiplexer in this
...
is the return value, in out case the wire connected to the compare function.

We use \code{unsignedBitLength} to specify the number of bits for the counter \code{cntReg}.
The number of bits to represent an unsigned number $n$ in binary is $\lfloor log_2(n) \rfloor + 1$.
Therefore, we could use following Chisel expression: \code{log2Floor(n)+1}.
However, with \code{unsignedBitLength} we do not need to remember if it is the ceiling or
floor function and if we need to add 1. Chisel also has a function
\code{signedBitLength} to provide the number of bits for a signed representation of a number.
...
The circuit \emph{shifts} the date on each clock from left to right. In this simple form the
circuit implements a 4-tap delay form \code{din} to \code{dout}.
...
is connected to one output bit. After 4 clock cycles this circuit converts a 4-bits serial data word
...
Figure~\ref{fig:shiftreg-paraout} shows a 4-bit shift register with a parallel output function.
