...
Thank you for moving out of the comfort zone and taking up the journey of
...
If we want to count N, we start with 0 and count till N-1. The following
counter counts from 0 to 100, inclusive.
...
up we need to compare against a number, which is a some bit string. The comparator
...
we use another language, e.g., Java or Python, to generate hardware.
...
In Section~\ref{sec:counter} we have explored various different forms of counters,
...
\noindent As a first implementation of that abstract class we provide the \code{when} based
counter:

\shortlist{code/when_ticker.txt}
...
The tester waits to see the first occurrence of a tick (the start might be different for
different implementations) and then check that \code{tick} repeats every $n$ clock cycles.

\shortlist{code/ticker_tester.txt}

\noindent Note the type parameter \code{T <: Ticker} on the test bench to accept a
\code{Ticker} or any class that inherits from \code{Ticker}.
