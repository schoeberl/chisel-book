...
The fifth edition does not include major changes, but is a consolidation version.
...
We updated the Leros code snippets in the processor chapter to the actual version.
\todo{We should also show more Leros code.}


\todo{What else has been changed. There should be some new Leros stuff.}
...
\verylonglist{code/leros_alu.txt}{The Leros ALU.}{lst:leros-alu}
...
which is later fed partially into the ALU. The \code{DecodeOut Bundle} contains more fields,
not showing here. See the original Leros code base for the details.

\shortlist{code/leros_decode_bundle.txt}

\noindent We also define a companion object for the \code{Decode} class that includes a
