...
For the fifth edition we have upgraded to the actual Chisel version 3.5.6.
...
\todo{What else has been changed.}
...
Combinational values can be conditionally assigned, but need to assign a value in every branch
...
software development. A collection of unit tests can also used for
...
into a \code{Module()}, and assigning it a name. In our example \code{add} and \code{reg}.
In this example we give the output of the register component (\code{reg.io.q}) the name \code{count}.
...
to fit onto a page (or slide) and to not distract by too many details.
...
modern FPGA flip-flops contain a synchronous reset (and set) input to the D flip-flop to not waste
...
hat single cycle \code{risingEdge} signal to increment a counter. 
...
to store state can be described as a single FSM. However, it might
not be practical, for example, to try to describe your laptop as a single FSM.
...
and proper indexing. This code snipped is just to show how a Scala \code{for} loop can be used
...
The next function (\code{map}) converts (maps) our duple of a Chisel \code{UInt}
and a Scala \code{Int} to a tuple of two Chisel \code{UInt}s. The \code{reduce} function provides the generation of
