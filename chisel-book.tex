...
\noindent Bit fields are concatenated with the \code{\#\#} operator.\footnote{Note, that there is a \code{Cat} function available that performs the same
operation with \code{Cat(highByte, lowByte)}.}
...
bundle fields are merged to a singe bit vector.
...
the following chapter shows how
