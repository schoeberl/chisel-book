...
For example, in an assembly line of a car factory, each stage performs an specialized task,
such as engine installation. When that task is done, the car moves to the next stage.
This pipelining execute different tasks in parallel at different stages of a car manufacturing.

In digital design we can also use pipelining to speedup processing of a stream of data.
A prominent example of pipelining in digital design is building a pipelined microprocessor.
In that case, instructions flow through the pipeline performing one operation in each stage
...
are running in parallel in different stages.
...
California, Berkeley.
The focus on Wildcat is to provide readable Chisel code that can be used in education.
