...
\section{Bundle and Vec}

Chisel provides two constructs to group related signals: (1) a \code{Bundle} to group
signals of different types and (2) a \code{Vec} to represent a collection of signals.
\code{Bundle}s and \code{Vec}s can be arbitrary nested.
...
to array data structures in other programing languages.\footnote{The name \code{Array}
is already used in Scala.}
A \code{Vec} is cerated by calling the constructor with two parameters: the
...
type, we need to pass a prototype for the vector fields. When using our
Channel, which we defined above, we can create a vector of channels with:
...
In this section we explore some medium size digital designs, such as
