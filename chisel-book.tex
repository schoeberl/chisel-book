...
\begin{figure}
  \centering
  \includegraphics[scale=0.7]{figures/components}
...
\begin{figure}
  \centering
  \includegraphics[scale=0.7]{figures/alu}
...
We can delegate to the synthesize tool to extract and minimize the boolean equations.


\section{Encoding and Decoding}

Two basic circuits, best described in a table form are encoding and decoding.
Encoding means to translate a binary number in the range of $0 .. 2^{m-1}$ to a select of
one in a binary vector of $2^m$ positions.

\todo{draw and encoder and maybe a table.}

The following code introduces the \code{switch} statement of Chisel to describe an encoder:
...
This assignment will be active and optimized away by the backend tool.
...
In the example we use unsigned integers for the signals. A maybe clearer representation
of an encode circuit uses binary notation:

\input{code/encdec_encbin.txt}

\noindent Note that the \code{switch} statement is not part of the core Chisel language. Therefore,
we need to include the elements of the package \code{chisel.util}.
