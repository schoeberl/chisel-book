...
One could start to consider both interface sides in the buffer to be able to accept
...
\longlist{code/fifo_double_buffer.txt}{A FIFO with double buffer elements.}{lst:fifo:double:buffer}


Listing~\ref{lst:fifo:double:buffer} shows the double buffer. The outer part of
the FIFO, the connection of the buffer elements is the same as in the bubble FIFO
and is therefore not shown.
The only change is in the doubler buffer element. It contains two registers,
...
However, compared to a single bubble FIFO, a restart of the queue happens
immediately, as the stages switch from state \code{two} to \code{one}.

Note the double buffer FIFO needs double the number of registers then the
bubble FIFO, but can also store double the number of element when needed.
The fall through latency is the same.
