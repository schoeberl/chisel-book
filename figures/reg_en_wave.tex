\documentclass[multi=tikzpicture]{standalone}

\usepackage[T1]{fontenc}
\usepackage{pslatex}
\renewcommand*\ttdefault{txtt}
\usepackage{tikz-timing}
\usetikztiminglibrary[rising arrows]{clockarrows}
\usetikztiminglibrary{either}

\begin{document}

\begin{tikztimingtable}[
font=\tt,
timing/yunit=2.5ex,
timing/xunit=3ex,
timing/text format=\raisebox{.4ex}\strut\tt\Large,
timing/u/background/.style={fill=lightgray},
timing/e/background/.style={fill=lightgray}
]
{clock}     & [L] 14{C} \\
{enable} & .1H 6H          2L    4H    1.9H \\
{inVal}   & .1U 2D{2} 2D{3}      2D{5}   4D{2}   2D{7} 1.9D{4} \\
{regEnable}   & .1U 2U 2D{2} 2D{3}  4D{5}   2D{2}   1.9D{7}  \\
\extracode
\vertlines[semitransparent,semithick,blue]{2,4,6,8,10,12}
\node[blue,font=\sf] at (2,-7)  {A};
\node[blue,font=\sf] at (4,-7)  {B};
\node[blue,font=\sf] at (6,-7)  {C};
\node[blue,font=\sf] at (8,-7)  {D};
\node[blue,font=\sf] at (10,-7) {E};
\node[blue,font=\sf] at (12,-7) {F};
\endextracode
\end{tikztimingtable}

\end{document}
