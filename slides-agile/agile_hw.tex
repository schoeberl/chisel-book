...
\item Functions can include state (registers)
\end{itemize}
\begin{chisel}
...
\item Generate a logic table at hardware construction time
\end{itemize}
\end{frame}
...
\item Read an ELF file with a Java library
%\item Table with a \code{sin} function
\item More ideas?
\end{itemize}
\end{frame}

\begin{frame}[fragile]{Slides and Further Material}
\begin{itemize}
\item Slides and a full course, including labs
\item Material for the following tutorial
\item The slide sources are part of the Chisel book source
\item \url{https://github.com/schoeberl/agile-hw}
\item Or Google: schoeberl github agile
\end{itemize}

\begin{figure}
    \centering
    \includegraphics[scale=0.1]{agile-hw-qr-code}
\end{figure}
\end{frame}

\begin{frame}[fragile]{Part of ODE4EC-PIV}
\begin{itemize}
\item Topic is part of the Chips JU project ODE4EC-PIV
\item \url{https://ode4ec.eu/}
\item We have a PhD position on agile HW at DTU
\item \url{https://efzu.fa.em2.oraclecloud.com/hcmUI/CandidateExperience/da/sites/CX_2001/job/7982}
\item Deadline 18 September
\end{itemize}
\end{frame}

\begin{frame}[fragile]{Summary}
\begin{itemize}
\item Write reusable generators, not RTL
\item Chisel + Scala + functional programming = HW generators
\item We continue in room  1 with the tutorial
\item This includes hands-on writing Scala/Chisel HW generators
\end{itemize}
\end{frame}
...
\item You need Java 17+ and \code{sbt}
\item Clone or download the repository from:
\begin{itemize}
\item \url{https://github.com/schoeberl/agile-hw}
\end{itemize}
\item Start IntelliJ or VSC and follow the instructions from the \href{https://github.com/schoeberl/agile-hw/tree/main/lab0}{lab0} page 
\item \code{sbt test}
\item Explore the code; maybe change the example
\end{itemize}
\end{frame}

\begin{frame}[fragile]{Session 2}
\begin{itemize}
...
\begin{frame}{A Bit More Chisel}
\begin{itemize}
    \item We had some in the first 30'
    \item Here are some more
    \item Enough to start generators
\end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Chisel Summary}
\begin{itemize}
\item Chisel is a small language
\item Embedding it in Scala gives it power
\item We can write circuit generators
\item We can do co-simulation
\item We will continue with Scala
\end{itemize}
\end{frame}

\begin{frame}[fragile]{Is it Time for a Break?}
\begin{itemize}
\item Fill up with coffee
\end{itemize}
\end{frame}
...
\begin{itemize}
\item I assume that you installed all tools during lab0
\item Functional programming in Scala
...
\item The following code generates Verilog code
\end{itemize}
...
\item We have seen this in action with the Chisel \code{Mux}
...
\item A great time for hardware developers!
\item Write hardware generators and not just RTL
\item Chisel builds on the power of object-oriented and functional Scala
\item Hope you try to get started with Scala/Chisel generators
\item Thank you for this afternoon!
\end{itemize}
\end{frame}
