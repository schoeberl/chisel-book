...
\begin{frame}[fragile]{Levels of Parameterization}
\begin{itemize}
\item Fixed hardware
\item Value parameter: \code{width: Int}
\item Configuration object: \code{Config(...)}
\item Type parameter: \code{T <: Data}
\item Structural generation:
    map / tabulate / reduceTree / for
\item Architecture generator:
...
\item Functions can serve as lightweight modules
\end{itemize}
\end{frame}

\begin{frame}[fragile]{Lab Last Week}
\begin{itemize}
...
\item Easier for the user of the component
\end{itemize}
\end{frame}

\begin{frame}[fragile]{Factory Methods Cont.}
\begin{itemize}
...
\end{chisel}
\begin{itemize}
\item Hides module instantiation, wiring, and exposes only the result
\item Important generator/library-design pattern
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Then let us build some hardware
\end{itemize}
\end{frame}
...
\end{chisel}
\item The number and type of ports can be decided at elaboration time
\item The interface itself can depend on generator parameters
\end{itemize}
\end{frame}
...
\begin{itemize}
\item \code{fire()} is true if and only if \code{ready} and \code{valid} are asserted
\item \code{enq(data)} sets data and sets valid to true (no check on \code{ready})
...
\begin{frame}[fragile]{Methods on \code{DecoupledIO}}
\begin{itemize}
\item Distinction between a Scala method and generated hardware
\item \code{io.in.fire} looks like a software method invocation
\item But produces a Chisel \code{Bool} expression representing \code{ready \&\& valid}.
\end{itemize}
\end{frame}
...
\item Fairness and guaranteed progress, especially in real-time systems, is an issue
...
\item \code{RRArbiter} chooses in round robin
\begin{itemize}
\item No details on implementation
\item A single-cycle round-robin arbiter can have substantial combinational logic
\end{itemize}
...
\begin{itemize}
\item Bubble FIFO (was proposed in lab 3)
...
\item Each router has 4 + 1 ports: North, South, West, East, and local
...
\begin{frame}[fragile]{Now we are Generating}
\begin{itemize}
\item The NoC example is a real generator
\item This is what we have been working toward
\item We aren't just parameterizing widths anymore
\item Scala is generating the architecture
\item 11 lines of Scala generate an entire network structure
\end{itemize}
\end{frame}

\begin{frame}[fragile]{Generator Summary}
\begin{itemize}
\item Scala runs at elaboration time
\item Chisel objects describe the hardware being constructed
\item Generators turn parameters, types, and algorithms into hardware structure
\end{itemize}
\end{frame}
...
\item Distributing derived software can require making corresponding source available under the GPL
\item Some companies have restrictions on using GPL software
\end{itemize}
\item Apache
\begin{itemize}
\item Similar to BSD and MIT
...
\begin{itemize}
\item Pick a topic and fill in the Google Doc
\item Have some inspiration from \url{https://scottbeamer.net/cse228a-sp25/project/index.html}
\item Next week is your initial presentation
\item No lab
\item Start working on it
\end{itemize}
\end{frame}
...
\item Go open source!
\item Next week: guest lecture by Ioannis on LLM usage at Microchip
\end{itemize}
\end{frame}
