...
\item About exam, check questions on email from Rani 6 April
\item Have a poll on electronic exam: which used, or non of the two
\item We will do a test exam, so we all learn
\end{itemize}
\end{frame}
...
\item You report
\item Base function is a 10, extra functions for a 12
\end{itemize}
\item Written exam
\begin{itemize}
...
\begin{itemize}
\item I assume you will use those resource it for doing a pretty good exam result and not for cheating :-)
...
\begin{frame}[fragile]{Exam Logistics}
\begin{itemize}
\item This is new to most of us: we will manage
\item We will do at least one (better two) test exams to learn the system
\end{itemize}
\end{frame}

\begin{frame}[fragile]{Exam Topics}
\begin{itemize}
\item yyy
\end{itemize}
\end{frame}
...
\begin{frame}[fragile]{yyy}
\begin{itemize}
%\item Jeg ved at det er meget at sp¿rge om, men jeg ville gerne vide, om det er mugligt, at fŒ et eksempel pŒ hvordan digi examen kommer til at se ud, nu chisel bliver brugt og ikke VHDL. Kunne vi mŒske fŒ en opgave, der kunne havt v¾ret eksamens opgaven? AltsŒ en opgave med lige sŒ mange sp¿rgsmŒl der minder om den vi kommer til at have til examen?
