...
\end{itemize}
\begin{chisel}
val regInit = Reg(init = UInt(0, 8))
\end{chisel}
\begin{itemize}
\item With this forward declaration we later assign the next value
\end{itemize}
\begin{chisel}
regInit := regInit + UInt(1)
\end{chisel}
\begin{itemize}
\item Register can also simply be defined within an expression
\end{itemize}
\begin{chisel}
...
\begin{itemize}
\item Our Chisel knowledge is complete to implement any digital circuit
\item Maybe not in the most elegant way ;-)
\item A counter is a simple basic component
\end{itemize}
\begin{chisel}
  val regCnt = Reg(init = UInt(0, 8))
  
  regCnt := Mux(regCnt === UInt(100),
    UInt(0), regCnt + UInt(1))
\end{chisel}
\end{frame}
...
\begin{itemize}
\item Some more code examples, e.g. counter with a register and a multuplexer
\item Bundles
\item Ports
\item Modules
\item Vecs
