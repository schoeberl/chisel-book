...
\begin{frame}[fragile]{Feedback Fixes}
\begin{itemize}
\item Changed ``Hello World'' example to use a 32-bit counter
\begin{itemize}
\item No point in saving a few bits in an intro example
\end{itemize}
\item Better syntax for \code{Vec} creation
\end{itemize}
\begin{chisel}
...
\end{chisel}
\begin{itemize}
\item Suggestion of a register file as \code{Reg} of a vector
\end{itemize}
\begin{chisel}
...
\begin{frame}[fragile]{More Feedback}
\begin{itemize}
\item Mux with more inputs (instead of \code{switch}):
\end{itemize}
\begin{chisel}
...
\begin{frame}[fragile]{More Feedback}
\begin{itemize}
\item Question on wiring up a component at creation time
\item Yes, by defining a function:
\end{itemize}
\begin{chisel}
...
\end{chisel}
\begin{itemize}
\item Or better as Factory method (will show it in the second session)
\end{itemize}
\begin{chisel}
val res2 = Adder(x, y)
\end{chisel}
...
\end{itemize}
% \item Do the UART exercise, which is mandatory
\end{itemize}
\end{frame}
